...
def generate_dana_payment_url(dana_number: str) -> str:
    ...
    # Gunakan library python-payment untuk membuat URL pembayaran
...
def payment_success(update: Update, context: CallbackContext) -> None:
    ...
    if payment_url:
        ...
        query.message.reply_text('Pembayaran berhasil! Tautan sekali join telah disalin.')

        # Hapus URL pembayaran dari variabel global setelah menyalin
        del context.user_data['payment_url']
    else:
        query.message.reply_text('Pembayaran berhasil!')


def main() -> None:
    """Run the bot."""
    # Inisialisasi Updater dan bot Telegram
